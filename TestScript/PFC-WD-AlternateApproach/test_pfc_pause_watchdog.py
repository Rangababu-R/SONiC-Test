...
from files.configs.pfc_wd import pfc_watch_dog_config, serializer, \
    pfc_watch_dog_multi_host_config, one_hundred_gbe, pfcwd_params
...
def test_pfc_watch_dog(api, 
    ...
    t_btwn_start_pause_and_stop_pause = pfcwd_params['t_stop_pause'] - pfcwd_params['t_start_pause']
    t_btwn_stop_pause_and_stop_traffic = pfcwd_params['t_stop_traffic'] - pfcwd_params['t_stop_pause']
    
    if pfcwd_params['storm_detection_time'] > (t_btwn_start_pause_and_stop_pause*1000):
        pytest_assert(False,
            "Storm Detection period should be less than time between Start Pause and Stop Pause")
        
    if  pfcwd_params['storm_restoration_time'] > (t_btwn_stop_pause_and_stop_traffic*1000):
        pytest_assert(False,
...
def test_multi_host_pfc_watch_dog(api, 
    ...
    pi_list = [prio for prio in lossless_prio_dscp_map]
    for pi in pi_list:
        config = pfc_watch_dog_multi_host_config(pi)
        api.set_config(None)
        # print(serializer.json(config))
        api.set_config(config)
